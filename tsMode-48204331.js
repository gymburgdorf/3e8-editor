...
import { t as R, m as E } from "./lib-eca73438.js";
/*!-----------------------------------------------------------------------------
...
var L = Object.defineProperty, H = Object.getOwnPropertyDescriptor, V = Object.getOwnPropertyNames, W = Object.prototype.hasOwnProperty, j = (e, t, r) => t in e ? L(e, t, { enumerable: !0, configurable: !0, writable: !0, value: r }) : e[t] = r, T = (e, t, r, l) => {
  if (t && typeof t == "object" || typeof t == "function")
    for (let s of V(t))
      !W.call(e, s) && s !== r && L(e, s, { get: () => t[s], enumerable: !(l = H(t, s)) || l.enumerable });
  return e;
}, B = (e, t, r) => (T(e, t, "default"), r && T(r, t, "default")), h = (e, t, r) => (j(e, typeof t != "symbol" ? t + "" : t, r), r), i = {};
B(i, E);
var U = class {
  constructor(e, t) {
...
var w = class {
  ...
  _textSpanToRange(e, t) {
    let r = e.getPositionAt(t.start), l = e.getPositionAt(t.start + t.length), { lineNumber: s, column: g } = r, { lineNumber: u, column: a } = l;
    return { startLineNumber: s, startColumn: g, endLineNumber: u, endColumn: a };
  }
}, $ = class {
  constructor(e) {
    _(this, "_libFiles");
  ...
  constructor(t, r, l, s) {
    ...
    this._disposables.push(i.editor.onDidCreateModel((n) => g(n))), this._disposables.push(i.editor.onWillDisposeModel(u)), this._disposables.push(i.editor.onDidChangeModelLanguage((n) => {
      u(n.model), g(n.model);
    ...
    this._disposables.push(this._defaults.onDidChange(a)), this._disposables.push(this._defaults.onDidExtraLibsChange(a)), i.editor.getModels().forEach((n) => g(n));
  }
  dispose() {
  ...
  async _doValidate(t) {
    ...
    if (t.isDisposed())
      return;
    const l = [], { noSyntaxValidation: s, noSemanticValidation: g, noSuggestionDiagnostics: u } = this._defaults.getDiagnosticsOptions();
    s || l.push(r.getSyntacticDiagnostics(t.uri.toString())), g || l.push(r.getSemanticDiagnostics(t.uri.toString())), u || l.push(r.getSuggestionDiagnostics(t.uri.toString()));
    const a = await Promise.all(l);
    if (!a || t.isDisposed())
      return;
    const n = a.reduce((d, f) => f.concat(d), []).filter((d) => (this._defaults.getDiagnosticsOptions().diagnosticCodesToIgnore || []).indexOf(d.code) === -1), p = n.map((d) => d.relatedInformation || []).reduce((d, f) => f.concat(d), []).map((d) => d.file ? i.Uri.parse(d.file.fileName) : null);
  ...
  _convertDiagnostics(t, r) {
    const l = r.start || 0, s = r.length || 1, { lineNumber: g, column: u } = t.getPositionAt(l), { lineNumber: a, column: n } = t.getPositionAt(l + s), p = [];
...
`),
  ...
  _convertRelatedInformation(t, r) {
    ...
    return r.forEach((s) => {
      let g = t;
      if (s.file && (g = this._libFiles.getOrCreateModel(s.file.fileName)), !g)
        return;
      const u = s.start || 0, a = s.length || 1, { lineNumber: n, column: p } = g.getPositionAt(u), { lineNumber: d, column: f } = g.getPositionAt(u + a);
      l.push({
        resource: g.uri,
        startLineNumber: n,
        startColumn: p,
...
`)
  ...
  async provideCompletionItems(e, t, r, l) {
    const s = e.getWordUntilPosition(t), g = new i.Range(t.lineNumber, s.startColumn, t.lineNumber, s.endColumn), u = e.uri, a = e.getOffsetAt(t), n = await this._worker(u);
    if (e.isDisposed())
      return;
    const p = await n.getCompletionsAtPosition(u.toString(), a);
    return !p || e.isDisposed() ? void 0 : {
      suggestions: p.entries.map((f) => {
        ...
        if (f.replacementSpan) {
          ...
          b = new i.Range(x.lineNumber, x.column, v.lineNumber, v.column);
        }
        const y = [];
  ...
  async resolveCompletionItem(e, t) {
    const r = e, l = r.uri, s = r.position, g = r.offset, a = await (await this._worker(l)).getCompletionEntryDetails(l.toString(), g, r.label);
    return a ? {
      uri: l,
      position: s,
      label: a.name,
...
var O = class extends w {
  ...
  async provideSignatureHelp(t, r, l, s) {
    const g = t.uri, u = t.getOffsetAt(r), a = await this._worker(g);
    if (t.isDisposed())
      return;
    const n = await a.getSignatureHelpItems(g.toString(), u, {
      triggerReason: O._toSignatureHelpTriggerReason(s)
    });
    if (!n || t.isDisposed())
      return;
    const p = {
  ...
  async provideHover(e, t, r) {
    const l = e.uri, s = e.getOffsetAt(t), g = await this._worker(l);
    if (e.isDisposed())
      return;
    const u = await g.getQuickInfoAtPosition(l.toString(), s);
    if (!u || e.isDisposed())
      return;
    const a = k(u.documentation), n = u.tags ? u.tags.map((d) => P(d)).join(`  
...
` + n : "")
  ...
  async provideDocumentHighlights(e, t, r) {
    ...
    if (e.isDisposed())
      return;
    const u = await g.getOccurrencesAtPosition(l.toString(), s);
    if (!(!u || e.isDisposed()))
      return u.map((a) => ({
        range: this._textSpanToRange(e, a.textSpan),
        kind: a.isWriteAccess ? i.languages.DocumentHighlightKind.Write : i.languages.DocumentHighlightKind.Text
      }));
  }
}, Q = class extends w {
  constructor(e, t) {
  ...
  async provideDefinition(e, t, r) {
    ...
    if (e.isDisposed())
      return;
    const u = await g.getDefinitionAtPosition(l.toString(), s);
    if (!u || e.isDisposed() || (await this._libFiles.fetchLibFilesIfNecessary(u.map((n) => i.Uri.parse(n.fileName))), e.isDisposed()))
      return;
    const a = [];
    for (let n of u) {
  ...
  async provideReferences(e, t, r, l) {
    const s = e.uri, g = e.getOffsetAt(t), u = await this._worker(s);
    if (e.isDisposed())
      return;
    const a = await u.getReferencesAtPosition(s.toString(), g);
  ...
  async provideDocumentSymbols(e, t) {
    ...
    if (e.isDisposed())
      return;
    const s = await l.getNavigationBarItems(r.toString());
    if (!s || e.isDisposed())
      return;
    const g = (a, n, p) => {
      let d = {
...
`,
  ...
  async provideDocumentRangeFormattingEdits(e, t, r, l) {
    ...
    if (e.isDisposed())
      return;
    const n = await a.getFormattingEditsForRange(s.toString(), g, u, S._convertOptions(r));
    if (!(!n || e.isDisposed()))
...
`];
  ...
  async provideOnTypeFormattingEdits(e, t, r, l, s) {
    const g = e.uri, u = e.getOffsetAt(t), a = await this._worker(g);
    if (e.isDisposed())
      return;
    const n = await a.getFormattingEditsAfterKeystroke(g.toString(), u, r, S._convertOptions(l));
    if (!(!n || e.isDisposed()))
      return n.map((p) => this._convertTextChanges(e, p));
  }
}, ee = class extends S {
  async provideCodeActions(e, t, r, l) {
    const s = e.uri, g = e.getOffsetAt({
      ...
      lineNumber: t.endLineNumber,
      column: t.endColumn
    }), a = S._convertOptions(e.getOptions()), n = r.markers.filter((b) => b.code).map((b) => b.code).map(Number), p = await this._worker(s);
    if (e.isDisposed())
      return;
    const d = await p.getCodeFixesAtPosition(s.toString(), g, u, n, a);
  ...
  _tsCodeFixActionToMonacoCodeAction(e, t, r) {
    const l = [];
    for (const g of r.changes)
      for (const u of g.textChanges)
        l.push({
          resource: e.uri,
          versionId: void 0,
          textEdit: {
  ...
  async provideRenameEdits(e, t, r, l) {
    const s = e.uri, g = s.toString(), u = e.getOffsetAt(t), a = await this._worker(s);
    if (e.isDisposed())
      return;
    const n = await a.getRenameInfo(g, u, {
    ...
    const p = await a.findRenameLocations(g, u, !1, !1, !1);
    if (!p || e.isDisposed())
      return;
    const d = [];
    for (const f of p) {
  ...
  async provideInlayHints(e, t, r) {
    const l = e.uri, s = l.toString(), g = e.getOffsetAt({
      lineNumber: t.startLineNumber,
      column: t.startColumn
    }), u = e.getOffsetAt({
      lineNumber: t.endLineNumber,
    ...
    return e.isDisposed() ? null : { hints: (await a.provideInlayHints(s, g, u)).map((d) => ({
      ...d,
      label: d.text,
